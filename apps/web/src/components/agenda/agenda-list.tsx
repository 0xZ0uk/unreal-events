...
import { AgendaEmpty, AgendaError, AgendaSkeleton, TruncationNote } from "./states";
...
function chipsFor(event: AgendaRow, activeCategory: string): string[] {
	const ordered = activeCategory
		? [...event.categories.filter((category) => category === activeCategory), ...event.categories.filter((category) => category !== activeCategory)]
		: event.categories;
	const chips = ordered.slice(0, 2);
	if (event.endAt !== null && dayKey(event.endAt) !== dayKey(event.startAt)) chips.unshift(`até ${shortDay(event.endAt)}`);
	return chips;
}
...
export function AgendaList({ agenda }: { agenda: Agenda }) {
	...
	if (agenda.status === "error") return <AgendaError message={agenda.error instanceof Error ? agenda.error.message : String(agenda.error)} onRetry={() => void agenda.refetch()} />;
	if (agenda.groups.length === 0) return <AgendaEmpty agenda={agenda} />;

	return (
		<div className="pt-8">
			{agenda.truncated ? <TruncationNote count={agenda.rows.length} days={agenda.window.days} /> : null}

			{agenda.groups.map((group) => (
				<section key={group.key} aria-labelledby={`dia-${group.key}`} className="mt-10 first:mt-0">
					<h2
						id={`dia-${group.key}`}
						className={`${GUTTER} sticky top-0 z-10 flex items-baseline justify-between gap-4 border-b border-border bg-background/90 py-3 backdrop-blur-sm`}
					>
						<span className="p443-card-title">
							{group.label}
							<span className="ml-2 font-mono text-[12px] font-normal tabular-nums text-muted-foreground">{group.date}</span>
						</span>
						<span className="shrink-0 font-mono text-[12px] tabular-nums text-muted-foreground">
							{group.events.length} {plural(group.events.length, "evento", "eventos")}
						</span>
					</h2>
